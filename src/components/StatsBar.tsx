...
import { Truck, CheckCircle, AlertTriangle, Package as PackageIcon, Clock } from 'lucide-react';
...
export default function StatsBar({ packages }: Props) {
  ...
  const overdue = packages.filter((p) => isOverdue(p)).length;
  ...
  const items = [
    ...
    { label: '已超时', value: overdue, icon: Clock, color: 'text-orange-400', bg: 'bg-orange-500/10' },
  ];
